...
void process_child_out_cb(socket_t *socket) {
	...
	char *str = json_pid_out(
	    sp->pid->pid, "stdout", socket->rbuf, socket->rbuf_len);
...
void process_http_cb(socket_t *socket) {
	...
	if (ptr->is_websocket) {
		...
		bcopy(
		    socket->rbuf, socket->rbuf + used, socket->rbuf_len - used);
		...
		if (cmd != NULL && cmd->type == CMD_EXECUTE) {
			...
			sprintf(
			    str, format, p->pid,
			    ((cmd_execute_t *)cmd)->command_str, time,
			    ((cmd_execute_t *)cmd)->request_id);
			...
			p->output = socket_new(
			    p->out, &process_child_out_cb, sp, socket);
			p->error = socket_new(
			    p->err, &process_child_out_cb, sp, socket);

			ev_child *child_watcher =
			    (struct ev_child *)malloc(sizeof(struct ev_child));
...
void child_cb(EV_P_ ev_child *w, int revents) {
	...
	sprintf(
	    str, format, w->rpid, WEXITSTATUS(w->rstatus), WSTOPSIG(w->rstatus),
	    time);
